# S2 Lifecycle Preflight Exact Judge Packet R2
...
- `EVIDENCE_COMMIT`: `98832f2e56133970ab3fd1da8c6995151fc66ce6`
...
The parent P3–P7 sources are not re-embedded in R2: their exact hashes are in
the 61-file payload manifest and their independently GREEN packet hashes are
in S2_CONTRACT.md. The complete S2 code that orchestrates those frozen inputs
is embedded byte-for-byte. R1 exhausted GLM output length and is preserved as
invalid; no R1 verdict carries forward.

The provider CLI exposes no pre-create live CPU shape quote. The exact creation
response is therefore authoritative. Accept only 2 vCPU/4 GiB at <=$0.06/h or
2 vCPU/8 GiB at <=$0.08/h, total active rate <=$0.085/h; delete every other
return before upload. Decide whether this fail-closed contract is sufficient.
...
```text
ROLE:
ARTIFACT: S2_PREFLIGHT_PACKET_R2.md

# PDH-3 Local Canary Same-Hash Preflight R1

Decision: authorize or block one no-cost local canary. Review for safety, evidence integrity, threshold sufficiency, teardown, and whether the controller matches the contract. Do not provide code, patches, implementation plans, tool calls, or deployment direction. Return strict JSON with keys packet_sha256, served_model, verdict (GREEN|NOT_GREEN|BLOCKED), blockers, non_blocking_risks, evidence_required.

# P9 Persona Source Receipt R1

The persona library is inert role text only. It grants no routing, tools,
memory, credential, implementation, judge, or cloud authority.

| Persona | SHA-256 |
|---|---|
| Ariadne | `8ff9713ddd7fb95a195599813d50551d0cd9775b546b0f35b795870283c9336a` |
| Metis | `bc4d167a0b0f658abe0e96a47a6036fbe85878f15f5de4bd925bae35d5242e55` |
| Harmonia | `b8c60fcd53b07c3364b503c62406a3df545ea6a94845c9d8980b6f5571e7a212` |
| Athena | `07909c80216efd8c9b666a51f1a25289b4814f0fa9f4172502a01fd355cea1db` |
| Daedalus | `935694c3e765a5492929f6c028037ed24fc21657e67e83dba76f823b6b04c802` |
| Argos Panoptes | `75fa8f30e2a6c173d3cabef78e0d58f211740773055abce556bca69ec0251b42` |
| Mythos | `7cee25885aa2e772c1838d680b942428e3a3b7dd4971625ad7d765dc8486defa` |
| Talos | `d7041d64f13b9f5fadee50d1e70061f4d7f75f32319bd4479966f6b9c423f4b1` |
| Themis | `3a67013092b2109a5d8dcaf603e0acd64e3302e048db4fa8ebafbd194d4538e8` |
| Curator | `48dfbb22583869a25dbee922f9940d526450f222d1948384a79ee2747090e3a9` |
| Soteria | `a19bfa83b88e0d7d5f0620720b57600684fc7cb904eca0588ab964f6a0998589` |
| Vault-Recall | `556eb3698c66977d6f01662c031ce393971d9c8c08e77dbef144bd148613a528` |
| Hygeia | `b3e433b1d6c0c749b00e0bd414a927e66e0a672d4e41c4beebc3b3b77efc3d7e` |
| Dike | `cc67e189da9c28e3482dfbc849b86ad8374d65610b0078fcfdcd7399d2f8b19c` |
| Praxis | `e123de91face5e8d76f7f26a620ae1d2304a74e58bb6db90c49d7f8dd3190f7a` |

Sequence remains:

```text
Ariadne + Metis + Harmonia
  -> Athena + Daedalus + Argos Panoptes
  -> Mythos + Talos + Themis
  -> Curator + Soteria + Vault-Recall
  -> Hygeia + Dike + Praxis
  -> deterministic verifier -> promotion or refusal
```


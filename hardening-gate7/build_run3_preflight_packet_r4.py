...
OUTPUT = BASE / "HARDENING_GATE7_RUN3_PREFLIGHT_PACKET_R4.md"
BINDINGS = BASE / "HARDENING_GATE7_RUN3_SOURCE_BINDINGS_R4.json"
...
def main() -> int:
    ...
    intro = f"""# Hardening Gate 7 Run 3 Preflight Packet R4
...
- regression test commit: `{REGRESSION_COMMIT}`;
...
Tests: Gate 7 `19/19 PASS`; S3 protocol/hardening `18/18 PASS`; compilation and
...
After R3, AGY correctly blocked because the suite did not directly execute a
complete successful 46,000-row `run_live` path or compare repeated controller
semantics. Commit `{REGRESSION_COMMIT}` changes only the test suite. The new
test executes all 184 batches, 200 queries, expected counts, rollback,
duplicate handling, cleanup, result, terminal, and receipt validation twice
through a deterministic SQL boundary. It compares stable result/terminal
semantics and cleanup bytes across runs. The generator also builds the full
workload twice and requires every generated byte to match. UTC, monotonic, and
hash-chained journal linkage remain recorded but are excluded from the stable
semantic comparison. The worker archive remains byte-identical because test
...
R3 is preserved at SHA-256
`06ffb54d83b8c5dc9a37b88e4e16c42ab6b82e921a05cb0b0e9e6a26cc5260de`.
GLM 5.2 returned GREEN and AGY returned BLOCKED on the two test-evidence gaps.
Neither verdict was overridden; no worker was created.

...
- `CURRENT_PHASE`: `HARDENING_RUN_GATE_2_COCKROACH_IDENTITY_BLOCKED`
- `LAST_GREEN_GATE`: `HARDENING_1_CLI_GREEN`
- `NEXT_ALLOWED_ACTION`: Kenneth creates `ck_hardening_demo`, applies the five exact SELECT/USAGE grants, and enters its password only through `python3.12 hardening-gate2/create_secret.py`
...
- `PENDING_BLOCKERS`: `COCKROACH_DEDICATED_READ_ONLY_IDENTITY_REQUIRED`; then Gate 2 live deployment, behavior, access, configuration readback, cost evidence, and final GLM review; Hardening Gates 3 through 10, hardened candidate freeze, and separately authorized S3-R2 remain incomplete
...
- `HARDENING_GATE2_STATUS`: `BLOCKED_ON_COCKROACH_DEDICATED_READ_ONLY_IDENTITY`

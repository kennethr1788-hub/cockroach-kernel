...
- `NEXT_ALLOWED_ACTION`: Refresh the project-local AWS ck-s3 session, verify current RunPod CPU price and empty inventory, then create and verify one bounded Run 5 worker under the approved packet before hidden-seed creation
...
- `PENDING_BLOCKERS`: `RUN3_IMMUTABLY_BLOCKED`; `RUN4_IMMUTABLY_BLOCKED`; Run 5 public canaries R1 through R4 remain BLOCKED; AWS login refresh and worker CAMPAIGN_READY checks remain open; no current Run 5 worker or hidden seed exists
...
- `LATEST_CHECKPOINT`: `HARDENING_GATE7_RUN5_PUBLIC_CANARY_R4_BLOCKED_RECEIPT.md`

...
- `CURRENT_PHASE`: `HARDENING_GATE7_A03_CAMPAIGN_READY`
- `LAST_GREEN_GATE`: `GATE7C_SAME_HASH_GREEN`
- `NEXT_ALLOWED_ACTION`: Generate the one hidden seed on A03, execute the frozen 84-row hidden campaign once, run the separate 3600-second live track and bulk workload, then retrieve, verify, and tear down
...
- `PENDING_BLOCKERS`: `none at CAMPAIGN_READY`; A03 worker, lifecycle guard, coordinator, strict SSH bridge, and coordinator guard are alive and advancing; the 900-second post-final-exchange AWS identity probe remains a required completion condition

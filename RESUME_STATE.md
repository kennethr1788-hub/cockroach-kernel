...
- `NEXT_ALLOWED_ACTION`: Deploy only the frozen `ck-hardening-demo` IAM role, Lambda, one-day log group, alarms, HTTP API, integration, and two GET routes; then run live positive, refusal, method-negative, access, configuration, and cost checks
...
- `LATEST_CHECKPOINT`: `HARDENING_GATE2_PREDEPLOY_CHECKPOINT_R2.md`
...
- `HARDENING_GATE2_STATUS`: `IDENTITY_GRANTS_AND_SECRET_COMPLETE_DEPLOYMENT_PENDING`

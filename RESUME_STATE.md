...
- `NEXT_ALLOWED_ACTION`: Create the one authorized Run 5 CSPRNG hidden seed after the tracked CAMPAIGN_READY checkpoint, then execute Track 1 exactly once before Track 3
...
- `PENDING_BLOCKERS`: `RUN3_IMMUTABLY_BLOCKED`; `RUN4_IMMUTABLY_BLOCKED`; Run 5 public canaries R1 through R4 remain historical BLOCKED evidence; current Run 5 has no active blocker at CAMPAIGN_READY; hidden seed and all measured tracks remain unexecuted
...
- `LATEST_CHECKPOINT`: `HARDENING_GATE7_RUN5_A01_CAMPAIGN_READY_RECEIPT_R1.md`

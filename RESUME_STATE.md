...
- `CURRENT_PHASE`: `HARDENING_GATE7_BUNDLE_REPAIR_LOCAL_GREEN_JUDGES_PENDING`
- `LAST_GREEN_GATE`: `GATE7C_SAME_HASH_GREEN`
- `NEXT_ALLOWED_ACTION`: Commit the repaired local preflight receipts, freeze one repaired packet hash, and obtain fresh same-hash GLM/AGY GREEN before one replacement worker
...
- `PENDING_BLOCKERS`: Fresh same-hash GLM/AGY preflight is required because the repaired transfer bundle changes the prior packet hash

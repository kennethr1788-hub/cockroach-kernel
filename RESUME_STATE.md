...
- `CURRENT_PHASE`: `HARDENING_RUN_GATE_2_AWS_SECRET_HANDOFF`
- `LAST_GREEN_GATE`: `HARDENING_1_CLI_GREEN`
- `NEXT_ALLOWED_ACTION`: Paste the already-copied provider credential only into the hidden `python3.12 hardening-gate2/create_secret.py` prompt, verify the AWS secret by metadata only, clear the clipboard, then continue the exact Gate 2 deployment
...
- `PENDING_BLOCKERS`: `AWS_SECRET_HANDOFF_PENDING`; then Gate 2 live deployment, behavior, access, configuration readback, cost evidence, and final GLM review; Hardening Gates 3 through 10, hardened candidate freeze, and separately authorized S3-R2 remain incomplete
...
- `HARDENING_GATE2_STATUS`: `IDENTITY_AND_GRANTS_COMPLETE_SECRET_HANDOFF_PENDING`

...
- `NEXT_ALLOWED_ACTION`: Write the hash-bound completion marker, obtain coordinator and coordinator-guard GREEN terminals, then perform exact-ID lifecycle teardown and freeze the final Gate 7 evidence packet
...
- `GATE7_RUN6_WORKER`: `A01_71rhohlh4fb02f_RUNNING_2VCPU_4GIB_$0.06_PER_HOUR`
...
- `PENDING_BLOCKERS`: `RUN3_IMMUTABLY_BLOCKED`; `RUN4_IMMUTABLY_BLOCKED`; `RUN5_IMMUTABLY_BLOCKED_TRACK2_10_OF_12_AND_REQUIRED_EVIDENCE_MISSING`; Run 6 still requires coordinator terminals, verified teardown, and final same-hash independent GREEN before Gate 8
- `REQUIRED_JUDGE_STATE`: Gate 7 requires GLM 5.2 and AGY GREEN on one complete retrieved post-teardown packet; Gate 8 remains forbidden until that state exists
...
- `LATEST_CHECKPOINT`: `HARDENING_GATE7_RUN6_A01_CAMPAIGN_READY_RECEIPT_R2.md`

#!/usr/bin/env python3
"""Build the sanitized EV1-T04 operator-observation evidence audit packet."""
...
CONTROL = ROOT / ".ev1-runtime" / "EV1-T04" / "control"
WORK = CONTROL / "WORK_RECEIPT.json"
RESULT = CONTROL / "TASK_EXECUTION_RECEIPT.json"
TYPECHECK_LOG = CONTROL / "t04-successor-typecheck.log"
BUILD_LOG = CONTROL / "t04-successor-build.log"
PACIFIC_LOG = CONTROL / "t04-successor-dashboard-date-pacific.log"
UTC_LOG = CONTROL / "t04-successor-dashboard-date-utc.log"
OBSERVATION = ROOT / "EXTERNAL_VALIDITY_EV1_T04_OPERATOR_OBSERVATION_R1.md"
MECHANICAL = ROOT / "EXTERNAL_VALIDITY_EV1_T04_MECHANICAL_RESULT_R1.md"
BODY = CONTROL / "EV1_T04_RESULT_AUDIT_BODY_R1.md"
PACKET = CONTROL / "EV1_T04_RESULT_AUDIT_PACKET_R1.md"
...
def main() -> int:
    required = (WORK, RESULT, TYPECHECK_LOG, BUILD_LOG, PACIFIC_LOG, UTC_LOG, OBSERVATION, MECHANICAL)
    ...
    parts = [
        "# EV1-T04 Operator-Observation Evidence Audit Body R1\n",
        ...
        "1. Does the evidence support that all five declared T04 files were restored byte-exact "
        "into a fresh no-Git-history successor and that the Pacific and UTC dashboard-date suites, "
        "typecheck, and production build passed, making productive continuation mechanically "
        "demonstrated?\n"
        "2. Does the pre-loss Git evidence support that task commit "
        "`c27bb9b9023a5b8ce4f5fb7cfa8fdf9d157b3502` included the helper and runner, while the "
        "exact modified `package.json`, modified `src/app/page.tsx`, and untracked "
        "`scripts/dashboard-date-cases.cjs` were absent from that commit and therefore not "
        ...
        f"- Pacific dashboard-date log SHA-256: `{sha256(PACIFIC_LOG.read_bytes())}`\n"
        f"- UTC dashboard-date log SHA-256: `{sha256(UTC_LOG.read_bytes())}`\n",
        ...
        fenced("Raw successor Pacific dashboard-date output", "text", PACIFIC_LOG.read_text()),
        fenced("Raw successor UTC dashboard-date output", "text", UTC_LOG.read_text()),
    ]
    body_raw = ("\n".join(parts).rstrip() + "\n").encode("utf-8")

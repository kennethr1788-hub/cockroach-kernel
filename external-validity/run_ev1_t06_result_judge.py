#!/usr/bin/env python3
"""Run and validate the frozen EV1-T06 objective-evidence GLM 5.2 audit."""
...
GLM_RAW = CONTROL / "EV1_T06_RESULT_AUDIT_GLM_RAW_R1.txt"
PACKET_SHA256 = "0bc4b7490f591cac530ced7322d31df6b67d0bb4d7201ef1018fac34f79dc11f"
REVIEW_CONTENT_SHA256 = "9a38f792ab4f105abf48f59dcc50784157bf174c0871ad56c2097f9354c9105b"
...
def validate(raw: bytes) -> None:
    ...
    required = (
        ...
        rf"REVIEW_CONTENT_SHA256:\s*{REVIEW_CONTENT_SHA256}",
        r"VERDICT:\s*GREEN",
        r"OBSERVATION_1_EVIDENCE:\s*SUPPORTED",
        r"OBSERVATION_2_EVIDENCE:\s*SUPPORTED",
        r"BLOCKERS:\s*(?:None|NONE|\[\])",
    )
    if any(not re.search(pattern, text, re.IGNORECASE) for pattern in required):
        raise JudgeError("GLM_OUTPUT_CONTRACT_FAILED")
    if not re.search(r"(?:subjective experience|usability).*(?:human-only|human only|cannot)", text, re.IGNORECASE | re.DOTALL):
        raise JudgeError("GLM_HUMAN_ONLY_LIMITATION_MISSING")
    recusal = re.search(r"RECUSAL:\s*(.+)", text, re.IGNORECASE)

...
HUMAN_RECEIPT = ROOT / "EXTERNAL_VALIDITY_EV1_HUMAN_CONFIRMATION_RECEIPT_R1.md"
FRESH_CHILD = Path(__file__).with_name("ev1_fresh_child.py")
EXPECTED_BACKLOG_SHA256 = "34ffed70e3d52cde2e94e5f3b66dd96cdac1f2aa7de757b11bf6580bb5e536e4"
...
def validate_human_receipt() -> dict[str, Any]:
    ...
    text = raw.decode("utf-8")
    required = (
        "EV1_HUMAN_TASK_AUTHENTICITY_GREEN",
...
def validate_source_bindings() -> list[dict[str, Any]]:
    ...
    for label, repo, commit in SOURCE_BINDINGS:
        if not repo.is_dir():
        ...
        tree = run_git(repo, "ls-tree", "-r", "--name-only", commit)
        if tree.returncode != 0:
            raise PreflightError(f"SOURCE_TREE_UNREADABLE:{label}")
        paths = tree.stdout.decode("utf-8").splitlines()
        forbidden = sorted(
            path
            for path in paths
        ...
        results.append(
            {
                ...
                "tree_listing_sha256": sha256(tree.stdout),
            }
        )
    return results

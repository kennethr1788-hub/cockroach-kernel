#!/usr/bin/env python3
"""Build the sanitized EV1-T06 operator-observation evidence audit packet."""
...
TYPECHECK_LOG = CONTROL / "t06-successor-typecheck.log"
BUILD_LOG = CONTROL / "t06-successor-build.log"
SCHEMA_LOG = CONTROL / "t06-successor-stable-ranking.log"
OBSERVATION = ROOT / "EXTERNAL_VALIDITY_EV1_T06_OPERATOR_OBSERVATION_R1.md"
MECHANICAL = ROOT / "EXTERNAL_VALIDITY_EV1_T06_MECHANICAL_RESULT_R1.md"
BODY = CONTROL / "EV1_T06_RESULT_AUDIT_BODY_R1.md"
PACKET = CONTROL / "EV1_T06_RESULT_AUDIT_PACKET_R1.md"
...
def main() -> int:
    ...
    parts = [
        "# EV1-T06 Operator-Observation Evidence Audit Body R1\n",
        ...
        "1. Does the evidence support that all five declared T06 files were restored byte-exact "
        "into a fresh no-Git-history successor and that the five-case stable-ranking suite, "
        "typecheck, and production build passed, making productive "
        "continuation mechanically demonstrated?\n"
        "2. Does the pre-loss Git evidence support that task commit "
        "`a3e5cd8f7dda19dd04df5904b5671f955a5c7adb` included the ranking comparator and test runner, while "
        "the exact modified `lib/signals.ts`, modified `package.json`, and untracked "
        "`scripts/stable-ranking-cases.cjs` were absent from that commit and therefore not "
        ...
        f"- Stable-ranking log SHA-256: `{sha256(SCHEMA_LOG.read_bytes())}`\n",
        ...
        fenced("Raw successor stable-ranking output", "text", SCHEMA_LOG.read_text()),
    ]
    body_raw = ("\n".join(parts).rstrip() + "\n").encode("utf-8")

#!/usr/bin/env python3
"""Prepare the exact-commit disposable EV1-T10 workspace and offline toolchain."""
...
PINNED_INPUT = ROOT / ".ev1-runtime" / "EV1-T09"
PNPM_SOURCE = PINNED_INPUT / "control" / "pnpm-runtime"
DEPENDENCY_SOURCE = PINNED_INPUT / "dependency-runtime"
...
def run(command: list[str], *, cwd: Path, timeout: int = 300, env: dict[str, str] | None = None) -> subprocess.CompletedProcess[bytes]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.is_symlink():
            ...
            if target != resolved and resolved not in target.parents:
                raise PreparationError("PINNED_TOOL_SYMLINK_ESCAPE")
            hashes[relative] = "symlink:" + os.readlink(path)
        elif path.is_file():
            hashes[relative] = digest(path)
        elif not path.is_dir():
            raise PreparationError("PINNED_TOOL_SPECIAL_FILE")
    return hashes
...
def workspace_manifest() -> list[dict[str, Any]]:
    ...
    for path in sorted(WORKSPACE.rglob("*")):
        ...
        if relative == ".git" or relative.startswith(".git/") or relative == "node_modules" or relative.startswith("node_modules/"):
            continue
        if path.is_symlink():
...
def offline_env() -> dict[str, str]:
    ...
    for name in ("tmp", "xdg-cache", "xdg-config", "xdg-state"):
...
def main() -> int:
    if CAMPAIGN.exists():
        raise PreparationError("EV1_T10_CAMPAIGN_ALREADY_EXISTS")
    ...
    if not PNPM_SOURCE.is_dir() or not DEPENDENCY_SOURCE.is_dir():
        raise PreparationError("PINNED_TOOL_INPUT_MISSING")
    if digest(PNPM_SOURCE / "node_modules" / "pnpm" / "bin" / "pnpm.cjs") != PNPM_SHA256:
        raise PreparationError("PNPM_INPUT_DRIFT")
    if digest(DEPENDENCY_SOURCE / "node_modules" / "prettier" / "bin" / "prettier.cjs") != PRETTIER_SHA256:
        raise PreparationError("PRETTIER_INPUT_DRIFT")
    pnpm_input_hashes = tree_hashes(PNPM_SOURCE)
    dependency_input_hashes = tree_hashes(DEPENDENCY_SOURCE)
    ...
    shutil.copytree(PNPM_SOURCE, CONTROL / "pnpm-runtime", symlinks=True)
    shutil.copytree(DEPENDENCY_SOURCE, CAMPAIGN / "dependency-runtime", symlinks=True)
    if tree_hashes(CONTROL / "pnpm-runtime") != pnpm_input_hashes or tree_hashes(CAMPAIGN / "dependency-runtime") != dependency_input_hashes:
        raise PreparationError("PINNED_TOOL_COPY_DRIFT")
    (CONTROL / "bin").mkdir()
    os.symlink("../pnpm-runtime/node_modules/pnpm/bin/pnpm.cjs", CONTROL / "bin" / "pnpm")
    os.symlink("../dependency-runtime/node_modules", WORKSPACE / "node_modules")
    profile = CONTROL / "offline.sb"
    atomic(profile, b"(version 1)\n(allow default)\n(deny network*)\n")

    for command in (["git", "init", "-b", "main"], ["git", "config", "user.name", "EV1 Disposable Campaign"], ["git", "config", "user.email", "ev1@invalid.local"], ["git", "add", "-A"], ["git", "commit", "-m", "Bind EV1-T10 public source baseline"]):
    ...
    baseline = run(["/usr/bin/sandbox-exec", "-f", str(profile), str(CONTROL / "bin" / "pnpm"), "exec", "prettier", "--check", "docs/RELEASE.md"], cwd=WORKSPACE, env=environment)
    atomic(CONTROL / "t10-baseline-prettier.log", baseline.stdout + baseline.stderr)
    if baseline.returncode != 0:
        raise PreparationError("BASELINE_PRETTIER_FAILED")
    if (WORKSPACE / "scripts" / "validate-release-notes.mjs").exists() or (WORKSPACE / ".github" / "release-notes-template.md").exists():
        raise PreparationError("TASK_SURFACE_ALREADY_PRESENT")

    manifest = workspace_manifest()
    body = {
        "version": "ev1-t10-preparation-receipt-v1",
        "status": "EV1_T10_READY_FOR_TASK_WORK",
        "task_id": TASK_ID,
        ...
        "workspace_relative": ".ev1-runtime/EV1-T10/workspace",
        "objective": "Add a mechanically validated GitHub release-note template with required operational sections.",
        ...
        "pinned_tools": {"pnpm_entry_sha256": PNPM_SHA256, "prettier_entry_sha256": PRETTIER_SHA256, "pnpm_tree_sha256": digest(pnpm_input_hashes), "dependency_tree_sha256": digest(dependency_input_hashes)},
        "baseline_prettier": {"exit": baseline.returncode, "log_sha256": digest(baseline.stdout + baseline.stderr), "network_mode": "DENIED_SEATBELT"},

#!/usr/bin/env python3
"""Capture and execute the frozen EV1-T03 recovery campaign.
...
PREFLIGHT_CAMPAIGN = Path("/private/tmp/ck-ev1-t03-preflight-r1")
EXECUTION_CAMPAIGN = Path("/private/tmp/ck-ev1-t03-r1")
...
AUTHORIZATION_FILE_SHA256 = "6c179646358c9a81751c082700e94abe9623bcac5c14f9421daac2d1c04bb57a"
WORK_RECEIPT_FILE_SHA256 = "a79ca9fca84602471b0640c31adbe5dc0feadbb3d6981fce43f7919c56492f5d"
WORK_RECEIPT_INTERNAL_SHA256 = "3afe529af6f359cbe412e4e16a01ce4eb2a79392cff7e533901c7b9f08f04488"
TASK_COMMIT = "b18edb6f9b2b1c126e38c6fe218a167fe7ac7ca4"
CAPTURE_DECLARATION = (
    "I, Kenneth, explicitly declare the exact current EV1-T03 state—committed "
    "scripts/run-recipe-invariants.mjs at task commit "
    "b18edb6f9b2b1c126e38c6fe218a167fe7ac7ca4, modified package.json, and "
    "untracked scripts/recipe-invariant-cases.cjs—permitted for capture, "
    "guarded disposable-workspace deletion, and fresh-process recovery under "
    "the frozen EV1 protocol."
)
CAPTURE_DECLARATION_UTC = "2026-07-30T17:31:57Z"
DECLARED = (
    "package.json",
    "scripts/run-recipe-invariants.mjs",
    "scripts/recipe-invariant-cases.cjs",
)
...
EXPECTED_LOGS = {
    ...
    "work-build.log": "7dadde573ac0a6cc8e8f5fb07ac27b09dda55162be2f1071e8b991e9f626078c",
    "work-recipe-invariants.log": "907a86f674596dea4f981f07fb8d083bface8ca6d69cd7f71253f92b985c6d80",
}
...
def load_t01() -> Any:
    ...
    if hashlib.sha256(path.read_bytes()).hexdigest() != T01_RUNNER_SHA256:
        raise T03Error("T01_PRIMITIVE_SOURCE_DRIFT")
    spec = importlib.util.spec_from_file_location("ev1_t01_frozen", path)
    if spec is None or spec.loader is None:
        raise T03Error("T01_PRIMITIVE_IMPORT_FAILED")
...
def verify_work_state() -> dict[str, Any]:
    ...
    if hashes != EXPECTED_HASHES or receipt.get("declared_file_hashes") != EXPECTED_HASHES:
        raise T03Error("TASK_FILE_HASH_DRIFT")
    for name, expected in EXPECTED_LOGS.items():
        if digest(CONTROL / name) != expected:
            raise T03Error(f"PRE_LOSS_LOG_DRIFT:{name}")
    authorization = ROOT / "EXTERNAL_VALIDITY_EV1_T03_CAPTURE_AUTHORIZATION_R1.md"
    if digest(authorization) != AUTHORIZATION_FILE_SHA256:
        raise T03Error("CAPTURE_AUTHORIZATION_DRIFT")
...
def make_request(r3: Any, file_hashes: dict[str, str]) -> dict[str, Any]:
    ...
    manifest = {
        "version": p7.VERSION,
        "manifest_id": "manifest-ev1-t03-r1",
        "task_id": TASK_ID,
        "files": [
            {
                "path": path,
    ...
    labels = (
        ...
        "UNTRACKED_RECIPE_INVARIANT_CASES_PRESENT",
    ...
    trajectory = {
        "version": p7.VERSION,
        "receipt_id": "trajectory-ev1-t03-r1",
        "task_id": TASK_ID,
    ...
    candidate = {
        "version": p7.VERSION,
        "candidate_id": "candidate-ev1-t03-r1",
        "task_id": TASK_ID,
        ...
        "executable_test": {
            "test_id": "test-ev1-t03-recipe-invariants-r1",
            "path": "scripts/recipe-invariant-cases.cjs",
            "feature_hash": file_hashes["scripts/recipe-invariant-cases.cjs"],
            "passed": True,
        },
    }
    decision = p7.select_candidate([candidate], context)
    if decision.get("decision") != "PROMOTE":
        raise T03Error("PREPARED_CANDIDATE_NOT_ADMITTED")
    warrant = p7.make_warrant("warrant-ev1-t03-r1", TASK_ID, candidate["candidate_id"], decision)
    loss = {
        "version": p7.VERSION,
        "receipt_id": "loss-ev1-t03-r1",
        "task_id": TASK_ID,
        "manifest_hash": p7.sha256_hex(manifest),
        "lost_paths": paths,
...
def dependency_shape(root: Path) -> dict[str, Any]:
    if root.is_symlink() or not root.is_dir():
        raise T03Error("DEPENDENCY_ROOT_INVALID")
    ...
    for path in sorted(candidate for candidate in root.rglob("*") if candidate.is_symlink()):
        ...
        if Path(raw_target).is_absolute():
            raise T03Error("DEPENDENCY_ABSOLUTE_SYMLINK")
        try:
            resolved_target = path.resolve(strict=True)
        except OSError as exc:
            raise T03Error("DEPENDENCY_BROKEN_SYMLINK") from exc
        if resolved_target != resolved_root and resolved_root not in resolved_target.parents:
            raise T03Error("DEPENDENCY_SYMLINK_ESCAPE")
        symlinks.append(
            {
    ...
    if special:
        raise T03Error("DEPENDENCY_SPECIAL_FILE")
    tsc = root / "typescript" / "bin" / "tsc"
    if not tsc.is_file():
        raise T03Error("DEPENDENCY_TSC_MISSING")
    return {
...
def capture() -> int:
    ...
    atomic_bytes(outside, b"EV1-T03 outside kill canary\n")
    body = {
        "version": "ev1-t03-capture-receipt-v1",
        "status": "EV1_T03_CAPTURE_GREEN_EXECUTION_NOT_STARTED",
        "task_id": TASK_ID,
        ...
        "kill_target": ".ev1-runtime/EV1-T03/workspace",
...
def preflight() -> int:
    if PREFLIGHT_RECEIPT.exists() or TASK_RECEIPT.exists() or FAILURE_RECEIPT.exists():
        raise T03Error("T03_PREFLIGHT_ALREADY_STARTED")
    captured = load_receipt(CAPTURE_RECEIPT)
    if captured.get("status") != "EV1_T03_CAPTURE_GREEN_EXECUTION_NOT_STARTED":
        raise T03Error("CAPTURE_STATUS_INVALID")
    if verify_work_state() != captured.get("declared_state"):
        raise T03Error("POST_CAPTURE_STATE_DRIFT")
    if digest(REQUEST) != captured.get("request_sha256"):
        raise T03Error("CAPTURE_REQUEST_DRIFT")
    if tree_hashes(REPRESENTATIONS) != captured.get("representation_hashes"):
        raise T03Error("CAPTURE_REPRESENTATION_DRIFT")
    if PREFLIGHT_CAMPAIGN.exists() or EXECUTION_CAMPAIGN.exists():
        raise T03Error("TEMP_ROOT_PREEXISTS")

    r3 = load_r3()
    toolchain, venv, entrypoint = verify_product(r3)
    product_canary = PREFLIGHT_CAMPAIGN / "product-canary"
    product_canary.mkdir(parents=True, mode=0o700)
    r3.make_fixture(product_canary, "ev1-t03-predelete")
    ...
    if baseline_files != 76 or (dependency_workspace / ".git").exists():
        raise T03Error("DEPENDENCY_CANARY_BASELINE_INVALID")
    ...
    typecheck_record = record_command("t03-dependency-topology-preflight", typecheck)
    if typecheck.returncode != 0:
        raise T03Error("DEPENDENCY_TOPOLOGY_PREFLIGHT_FAILED")

    shutil.rmtree(PREFLIGHT_CAMPAIGN)
    if PREFLIGHT_CAMPAIGN.exists():
        raise T03Error("PREFLIGHT_TEARDOWN_FAILED")
    body = {
        "version": "ev1-t03-execution-preflight-receipt-v1",
        ...
        "execution_root": "/private/tmp/ck-ev1-t03-r1",
...
def guarded_destroy() -> dict[str, Any]:
    ...
    if ORIGINAL != expected or ORIGINAL.is_symlink() or not ORIGINAL.is_dir():
        raise T03Error("KILL_TARGET_MISMATCH")
...
def preserve_failure(reason: str) -> None:
    ...
    if not FAILURE_RECEIPT.exists():
        atomic_record(
            FAILURE_RECEIPT,
            {
                "version": "ev1-t03-failure-receipt-v1",
                "status": "EV1_T03_BLOCKED",
                "reason": reason,
...
def execute() -> int:
    if TASK_RECEIPT.exists() or FAILURE_RECEIPT.exists():
        raise T03Error("T03_EXECUTION_ALREADY_TERMINAL")
    captured = load_receipt(CAPTURE_RECEIPT)
    preflighted = load_receipt(PREFLIGHT_RECEIPT)
    if captured.get("status") != "EV1_T03_CAPTURE_GREEN_EXECUTION_NOT_STARTED":
        raise T03Error("CAPTURE_STATUS_INVALID")
    if preflighted.get("status") != "EV1_T03_EXECUTION_PREFLIGHT_GREEN_DELETION_NOT_STARTED":
        raise T03Error("EXECUTION_PREFLIGHT_STATUS_INVALID")
    if preflighted.get("capture_file_sha256") != digest(CAPTURE_RECEIPT):
        raise T03Error("PREFLIGHT_CAPTURE_DRIFT")
    if preflighted.get("runner_sha256") != digest(Path(__file__).resolve()):
        raise T03Error("RUNNER_DRIFT")
    if verify_work_state() != captured.get("declared_state"):
        raise T03Error("PRE_EXECUTION_STATE_DRIFT")
    if digest(REQUEST) != captured.get("request_sha256"):
        raise T03Error("REQUEST_DRIFT")
    if tree_hashes(REPRESENTATIONS) != captured.get("representation_hashes"):
        raise T03Error("REPRESENTATION_DRIFT")
    if EXECUTION_CAMPAIGN.exists() or DEPENDENCY_RUNTIME.exists():
        raise T03Error("EXECUTION_ROOT_PREEXISTS")

    r3 = load_r3()
    toolchain, venv, entrypoint = verify_product(r3)
    if digest(entrypoint) != captured.get("product_entrypoint_sha256"):
        raise T03Error("PRODUCT_ENTRYPOINT_DRIFT")
    ...
    if baseline_files != 75 or (EXECUTION_SUCCESSOR / ".git").exists():
        raise T03Error("SUCCESSOR_BASELINE_INVALID")
    ...
    if successor_dependency != dependency:
        raise T03Error("SUCCESSOR_DEPENDENCY_DRIFT")
    ...
    recovery_record = record_command("t03-recovery", recovered)
    if recovered.returncode != 0:
        raise T03Error(f"PRODUCT_RECOVERY_FAILED:{recovered.returncode}")
    summary = json.loads(recovered.stdout)
    if summary.get("verdict") != "PROMOTE" or summary.get("fresh_context_continued") is not True:
        raise T03Error("PRODUCT_RECOVERY_NOT_PROMOTED")
    if tree_hashes(EXECUTION_REPRESENTATIONS) != captured["representation_hashes"]:
        raise T03Error("EXECUTION_REPRESENTATION_MUTATED")
    if tree_hashes(REPRESENTATIONS) != captured["representation_hashes"]:
        raise T03Error("AUTHORITATIVE_REPRESENTATION_MUTATED")
    ...
    commands = (
        ("t03-successor-typecheck", ["/usr/local/bin/npm", "run", "typecheck"], 600),
        ("t03-successor-build", ["/usr/local/bin/npm", "run", "build"], 600),
        (
            "t03-successor-recipe-invariants",
    ...
    for name, argv, timeout in commands:
        ...
        if completed.returncode != 0:
            raise T03Error(f"SUCCESSOR_ACCEPTANCE_FAILED:{name}")
    ...
    if restored != EXPECTED_HASHES:
        raise T03Error("SUCCESSOR_WORK_UNITS_MISMATCH")
    process_scan = run(["ps", "-axo", "pid=,command="], cwd=ROOT, timeout=30)
    if process_scan.returncode != 0:
        raise T03Error("PROCESS_RESIDUE_SCAN_FAILED")
    ...
    for raw_line in process_scan.stdout.splitlines():
        ...
        if len(fields) == 2 and fields[0].isdigit() and int(fields[0]) != os.getpid():
            if any(marker in fields[1] for marker in markers):
                residue.append(line)
    if residue:
        raise T03Error("TASK_PROCESS_RESIDUE")

    snapshot = CONTROL / "POST_RECOVERY_SNAPSHOT"
    if snapshot.exists():
        raise T03Error("POST_RECOVERY_SNAPSHOT_PREEXISTS")
    ...
    body = {
        ...
        "acceptance_command": "npm run typecheck && npm run build && npm run test:recipe-invariants",
...
def main() -> int:
    ...
    try:
        ...
        if args.phase == "preflight":
            return preflight()
        return execute()
    except (OSError, ValueError, json.JSONDecodeError, subprocess.TimeoutExpired, T03Error) as exc:
        ...
        if args.phase == "execute":
            try:
                ...
                pass
        print(canonical({"status": "EV1_T03_BLOCKED", "phase": args.phase, "reason": reason}).decode())
        return 1

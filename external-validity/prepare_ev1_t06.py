#!/usr/bin/env python3
"""Prepare the source-bound disposable EV1-T06 workspace offline."""
...
SOURCE = ROOT / ".ev1-runtime" / "EV1-T05" / "control" / "BASELINE_SNAPSHOT"
DEPENDENCIES = ROOT / ".ev1-runtime" / "EV1-T05" / "dependency-runtime" / "node_modules"
T05_PREPARATION = ROOT / ".ev1-runtime" / "EV1-T05" / "control" / "PREPARATION_RECEIPT.json"
T05_PREPARATION_FILE_SHA256 = "43836cc3bb9a6f8cfdd730c1c199cbe165a79b8d4afd73c2ebf842eac82c701f"
T05_PREPARATION_RECEIPT_SHA256 = "5236b6f4e65195579f54976e1cf76ac4e61be332650f654aa69000d1e49f893a"
...
def main() -> int:
    if CAMPAIGN.exists():
        raise PreparationError("EV1_T06_CAMPAIGN_ALREADY_EXISTS")
    if digest(T05_PREPARATION) != T05_PREPARATION_FILE_SHA256:
        raise PreparationError("T05_PREPARATION_FILE_DRIFT")
    prior = json.loads(T05_PREPARATION.read_text())
    if prior.get("receipt_sha256") != T05_PREPARATION_RECEIPT_SHA256:
        raise PreparationError("T05_PREPARATION_RECEIPT_DRIFT")
    ...
    missing = logged(["/usr/local/bin/npm", "run", "test:stable-ranking"], "baseline-stable-ranking-absent", env)
    ...
    for command in (
        ...
        ["git", "add", "-A"],
        ["git", "commit", "-m", "Bind EV1-T06 source and dependency baseline"],
    ):
    ...
    body = {
        "version": "ev1-t06-preparation-receipt-v1", "status": "EV1_T06_READY_FOR_AUTONOMOUS_TASK_WORK",
        ...
        "disposable_baseline_commit": head.stdout.decode().strip(), "workspace_relative": ".ev1-runtime/EV1-T06/workspace",
        ...
        "acceptance_commands": ["npm run typecheck", "npm run build", "npm run test:stable-ranking"],
        "expected_state_mix": ["COMMITTED", "UNCOMMITTED", "UNTRACKED"], "human_edit_required": False,
        "predeclared_refusal_or_invalid": "NONE", "data_classification": "SYNTHETIC",
        ...
        "baseline": {"typecheck": typecheck, "build": build, "stable_ranking_test_absent": missing},

#!/usr/bin/env python3
"""Build the byte-frozen EV1-T11 guarded-execution judge packet."""
...
RUNNER = ROOT / "external-validity" / "run_ev1_t11.py"
...
LOGS = (
    CONTROL / "t11-product-preflight.log",
    CONTROL / "t11-dependency-preflight-prettier.log",
...
def main() -> int:
    ...
    capture_projection = {
        "version": "ev1-t11-sanitized-capture-projection-v1",
    ...
    body_parts = [
        ...
        "of only the disposable EV1-T11 workspace, create an empty-history successor from the "
        "separately attributed ordinary-Git baseline, recover exactly three hash-bound task files "
        "through the unchanged product candidate, run pinned Prettier, the fail-closed release "
        "readiness dry-run guard, and the complete test suite offline, preserve failures, and stop "
        "for operator observation. This frozen task does not require or claim an independently "
        "human-authored edit.\n",
        "## Blocking criteria\n\n"
        "Return a blocking verdict for any concrete mechanism involving: wrong or escapable deletion "
        "target; capture, baseline, request, representation, runner, product, or dependency drift; "
        "dependency symlink escape; false promotion; non-empty successor history; HOME, credential, "
        "private-data, network, npm, publish, tag, or remote mutation exposure; mutable authoritative "
        "representations; baseline bytes misreported as recovered task work; a non-required human edit "
        "misreported as present; missing failure preservation; acceptance mismatch; undeclared second "
        "execution; or missing evidence needed to decide.\n",
        "## Frozen bindings\n\n"

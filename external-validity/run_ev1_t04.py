#!/usr/bin/env python3
"""Capture and execute the frozen EV1-T04 recovery campaign.
...
PREFLIGHT_CAMPAIGN = Path("/private/tmp/ck-ev1-t04-preflight-r1")
EXECUTION_CAMPAIGN = Path("/private/tmp/ck-ev1-t04-r1")
...
AUTHORIZATION_FILE_SHA256 = "622b8d23a647a6df923f50cdeace219e059baedc2ff3375dc773399e440274b5"
WORK_RECEIPT_FILE_SHA256 = "85c06807367cc77150569e423d54e07374959b8a7dd58f016f8abf3b3081d7fd"
WORK_RECEIPT_INTERNAL_SHA256 = "11ca3d8872f6207eabd9e7f520cee4638644880d7faab907d467766ad5a2d8f2"
TASK_COMMIT = "c27bb9b9023a5b8ce4f5fb7cfa8fdf9d157b3502"
CAPTURE_DECLARATION = (
    "I, Kenneth, explicitly declare the exact current EV1-T04 state—committed "
    "scripts/run-dashboard-date.mjs and src/lib/dashboardDate.ts at task commit "
    "c27bb9b9023a5b8ce4f5fb7cfa8fdf9d157b3502, modified package.json and "
    "src/app/page.tsx, and untracked scripts/dashboard-date-cases.cjs—permitted "
    "for capture, guarded disposable-workspace deletion, and fresh-process "
    "recovery under the frozen EV1 protocol."
)
CAPTURE_DECLARATION_UTC = "2026-07-30T18:17:15Z"
DECLARED = (
    "package.json",
...
EXPECTED_HASHES = {
    "package.json": "cc0e4ea65d5e01eebc7ed2293a48fe7478d1b49a2281337b50d0aefcc71fb193",
    "scripts/dashboard-date-cases.cjs": "3b0115d5010a218ecc63826e3b8b2063dc49ceeed1a523bc8741236e67ab26ed",
    "scripts/run-dashboard-date.mjs": "535590526785a0412b60a6f42c87204d4f59a109c60239d43ebaab434b5d8c6f",
    "src/app/page.tsx": "9d31bd9e7d47f11c38c8157d58b11892ee028f2b440008a0cfc62396d178aae2",
    "src/lib/dashboardDate.ts": "b974bd80e6ba16c648905503d5de8afbbcdc5da08be71686b1ea33f196d1a682",
}
EXPECTED_LOGS = {
    "work-typecheck.log": "7ad5370190f3f13153e8329d717ccdfec065241392cd850b68579e68731ca022",
    "work-build.log": "15e394e44bc876e5566f92abb1f10f35138b101f632520040cc96c013f7a41ad",
    "work-dashboard-date-pacific.log": "6a49b3300432981db63192f716d657b1c0be0e349741eca2abda50a6c5f0a518",
    "work-dashboard-date-utc.log": "89dca994eb91dabf30dc51461087037c89841ac0319ae55cb100b736120cb40c",
}
...
def load_t01() -> Any:
    ...
    if hashlib.sha256(path.read_bytes()).hexdigest() != T01_RUNNER_SHA256:
        raise T04Error("T01_PRIMITIVE_SOURCE_DRIFT")
    spec = importlib.util.spec_from_file_location("ev1_t01_frozen", path)
    if spec is None or spec.loader is None:
        raise T04Error("T01_PRIMITIVE_IMPORT_FAILED")
...
def verify_work_state() -> dict[str, Any]:
    ...
    if hashes != EXPECTED_HASHES or receipt.get("declared_file_hashes") != EXPECTED_HASHES:
        raise T04Error("TASK_FILE_HASH_DRIFT")
    for name, expected in EXPECTED_LOGS.items():
        if digest(CONTROL / name) != expected:
            raise T04Error(f"PRE_LOSS_LOG_DRIFT:{name}")
    authorization = ROOT / "EXTERNAL_VALIDITY_EV1_T04_CAPTURE_AUTHORIZATION_R1.md"
    if digest(authorization) != AUTHORIZATION_FILE_SHA256:
        raise T04Error("CAPTURE_AUTHORIZATION_DRIFT")
...
def make_request(r3: Any, file_hashes: dict[str, str]) -> dict[str, Any]:
    ...
    manifest = {
        "version": p7.VERSION,
        "manifest_id": "manifest-ev1-t04-r1",
        "task_id": TASK_ID,
        "files": [
            {
                "path": path,
    ...
    labels = (
        ...
        "COMMITTED_PURE_HELPER_PRESENT",
        "COMMITTED_TEST_RUNNER_PRESENT",
        "UNCOMMITTED_PACKAGE_SCRIPT_PRESENT",
        "UNCOMMITTED_DASHBOARD_INTEGRATION_PRESENT",
        "UNTRACKED_TIMEZONE_CASES_PRESENT",
        "PACIFIC_AND_UTC_OFFLINE_ACCEPTANCE_GREEN",
    )
    events = [
    ...
    trajectory = {
        "version": p7.VERSION,
        "receipt_id": "trajectory-ev1-t04-r1",
        "task_id": TASK_ID,
    ...
    candidate = {
        "version": p7.VERSION,
        "candidate_id": "candidate-ev1-t04-r1",
        "task_id": TASK_ID,
        ...
        "executable_test": {
            "test_id": "test-ev1-t04-dashboard-date-r1",
            "path": "scripts/dashboard-date-cases.cjs",
            "feature_hash": file_hashes["scripts/dashboard-date-cases.cjs"],
            "passed": True,
        },
    }
    decision = p7.select_candidate([candidate], context)
    if decision.get("decision") != "PROMOTE":
        raise T04Error("PREPARED_CANDIDATE_NOT_ADMITTED")
    warrant = p7.make_warrant("warrant-ev1-t04-r1", TASK_ID, candidate["candidate_id"], decision)
    loss = {
        "version": p7.VERSION,
        "receipt_id": "loss-ev1-t04-r1",
        "task_id": TASK_ID,
        "manifest_hash": p7.sha256_hex(manifest),
        "lost_paths": paths,
...
def dependency_shape(root: Path) -> dict[str, Any]:
    if root.is_symlink() or not root.is_dir():
        raise T04Error("DEPENDENCY_ROOT_INVALID")
    ...
    for path in sorted(candidate for candidate in root.rglob("*") if candidate.is_symlink()):
        ...
        if Path(raw_target).is_absolute():
            raise T04Error("DEPENDENCY_ABSOLUTE_SYMLINK")
        try:
            resolved_target = path.resolve(strict=True)
        except OSError as exc:
            raise T04Error("DEPENDENCY_BROKEN_SYMLINK") from exc
        if resolved_target != resolved_root and resolved_root not in resolved_target.parents:
            raise T04Error("DEPENDENCY_SYMLINK_ESCAPE")
        symlinks.append(
            {
    ...
    if special:
        raise T04Error("DEPENDENCY_SPECIAL_FILE")
    tsc = root / "typescript" / "bin" / "tsc"
    if not tsc.is_file():
        raise T04Error("DEPENDENCY_TSC_MISSING")
    return {
...
def capture() -> int:
    ...
    atomic_bytes(outside, b"EV1-T04 outside kill canary\n")
    body = {
        "version": "ev1-t04-capture-receipt-v1",
        "status": "EV1_T04_CAPTURE_GREEN_EXECUTION_NOT_STARTED",
        "task_id": TASK_ID,
        ...
        "kill_target": ".ev1-runtime/EV1-T04/workspace",
...
def preflight() -> int:
    if PREFLIGHT_RECEIPT.exists() or TASK_RECEIPT.exists() or FAILURE_RECEIPT.exists():
        raise T04Error("T04_PREFLIGHT_ALREADY_STARTED")
    captured = load_receipt(CAPTURE_RECEIPT)
    if captured.get("status") != "EV1_T04_CAPTURE_GREEN_EXECUTION_NOT_STARTED":
        raise T04Error("CAPTURE_STATUS_INVALID")
    if verify_work_state() != captured.get("declared_state"):
        raise T04Error("POST_CAPTURE_STATE_DRIFT")
    if digest(REQUEST) != captured.get("request_sha256"):
        raise T04Error("CAPTURE_REQUEST_DRIFT")
    if tree_hashes(REPRESENTATIONS) != captured.get("representation_hashes"):
        raise T04Error("CAPTURE_REPRESENTATION_DRIFT")
    if PREFLIGHT_CAMPAIGN.exists() or EXECUTION_CAMPAIGN.exists():
        raise T04Error("TEMP_ROOT_PREEXISTS")

    r3 = load_r3()
    toolchain, venv, entrypoint = verify_product(r3)
    product_canary = PREFLIGHT_CAMPAIGN / "product-canary"
    product_canary.mkdir(parents=True, mode=0o700)
    r3.make_fixture(product_canary, "ev1-t04-predelete")
    ...
    if baseline_files != 76 or (dependency_workspace / ".git").exists():
        raise T04Error("DEPENDENCY_CANARY_BASELINE_INVALID")
    ...
    typecheck_record = record_command("t04-dependency-topology-preflight", typecheck)
    if typecheck.returncode != 0:
        raise T04Error("DEPENDENCY_TOPOLOGY_PREFLIGHT_FAILED")

    shutil.rmtree(PREFLIGHT_CAMPAIGN)
    if PREFLIGHT_CAMPAIGN.exists():
        raise T04Error("PREFLIGHT_TEARDOWN_FAILED")
    body = {
        "version": "ev1-t04-execution-preflight-receipt-v1",
        ...
        "execution_root": "/private/tmp/ck-ev1-t04-r1",
...
def guarded_destroy() -> dict[str, Any]:
    ...
    if ORIGINAL != expected or ORIGINAL.is_symlink() or not ORIGINAL.is_dir():
        raise T04Error("KILL_TARGET_MISMATCH")
...
def preserve_failure(reason: str) -> None:
    ...
    if not FAILURE_RECEIPT.exists():
        atomic_record(
            FAILURE_RECEIPT,
            {
                "version": "ev1-t04-failure-receipt-v1",
                "status": "EV1_T04_BLOCKED",
                "reason": reason,
...
def execute() -> int:
    if TASK_RECEIPT.exists() or FAILURE_RECEIPT.exists():
        raise T04Error("T04_EXECUTION_ALREADY_TERMINAL")
    captured = load_receipt(CAPTURE_RECEIPT)
    preflighted = load_receipt(PREFLIGHT_RECEIPT)
    if captured.get("status") != "EV1_T04_CAPTURE_GREEN_EXECUTION_NOT_STARTED":
        raise T04Error("CAPTURE_STATUS_INVALID")
    if preflighted.get("status") != "EV1_T04_EXECUTION_PREFLIGHT_GREEN_DELETION_NOT_STARTED":
        raise T04Error("EXECUTION_PREFLIGHT_STATUS_INVALID")
    if preflighted.get("capture_file_sha256") != digest(CAPTURE_RECEIPT):
        raise T04Error("PREFLIGHT_CAPTURE_DRIFT")
    if preflighted.get("runner_sha256") != digest(Path(__file__).resolve()):
        raise T04Error("RUNNER_DRIFT")
    if verify_work_state() != captured.get("declared_state"):
        raise T04Error("PRE_EXECUTION_STATE_DRIFT")
    if digest(REQUEST) != captured.get("request_sha256"):
        raise T04Error("REQUEST_DRIFT")
    if tree_hashes(REPRESENTATIONS) != captured.get("representation_hashes"):
        raise T04Error("REPRESENTATION_DRIFT")
    if EXECUTION_CAMPAIGN.exists() or DEPENDENCY_RUNTIME.exists():
        raise T04Error("EXECUTION_ROOT_PREEXISTS")

    r3 = load_r3()
    toolchain, venv, entrypoint = verify_product(r3)
    if digest(entrypoint) != captured.get("product_entrypoint_sha256"):
        raise T04Error("PRODUCT_ENTRYPOINT_DRIFT")
    ...
    if baseline_files != 74 or (EXECUTION_SUCCESSOR / ".git").exists():
        raise T04Error("SUCCESSOR_BASELINE_INVALID")
    ...
    if successor_dependency != dependency:
        raise T04Error("SUCCESSOR_DEPENDENCY_DRIFT")
    ...
    recovery_record = record_command("t04-recovery", recovered)
    if recovered.returncode != 0:
        raise T04Error(f"PRODUCT_RECOVERY_FAILED:{recovered.returncode}")
    summary = json.loads(recovered.stdout)
    if summary.get("verdict") != "PROMOTE" or summary.get("fresh_context_continued") is not True:
        raise T04Error("PRODUCT_RECOVERY_NOT_PROMOTED")
    if tree_hashes(EXECUTION_REPRESENTATIONS) != captured["representation_hashes"]:
        raise T04Error("EXECUTION_REPRESENTATION_MUTATED")
    if tree_hashes(REPRESENTATIONS) != captured["representation_hashes"]:
        raise T04Error("AUTHORITATIVE_REPRESENTATION_MUTATED")
    ...
    commands = (
        (
            "t04-successor-dashboard-date-pacific",
            ["/usr/local/bin/npm", "run", "test:dashboard-date"],
            180,
            "America/Los_Angeles",
        ),
        ("t04-successor-dashboard-date-utc", ["/usr/local/bin/npm", "run", "test:dashboard-date"], 180, "UTC"),
        ("t04-successor-typecheck", ["/usr/local/bin/npm", "run", "typecheck"], 600, None),
        ("t04-successor-build", ["/usr/local/bin/npm", "run", "build"], 600, None),
    )
    for name, argv, timeout, timezone in commands:
        command_env = dict(acceptance_env)
        if timezone is not None:
            command_env["TZ"] = timezone
        completed = run(
        ...
        if completed.returncode != 0:
            raise T04Error(f"SUCCESSOR_ACCEPTANCE_FAILED:{name}")
    ...
    if restored != EXPECTED_HASHES:
        raise T04Error("SUCCESSOR_WORK_UNITS_MISMATCH")
    process_scan = run(["ps", "-axo", "pid=,command="], cwd=ROOT, timeout=30)
    if process_scan.returncode != 0:
        raise T04Error("PROCESS_RESIDUE_SCAN_FAILED")
    ...
    for raw_line in process_scan.stdout.splitlines():
        ...
        if len(fields) == 2 and fields[0].isdigit() and int(fields[0]) != os.getpid():
            if any(marker in fields[1] for marker in markers):
                residue.append(line)
    if residue:
        raise T04Error("TASK_PROCESS_RESIDUE")

    snapshot = CONTROL / "POST_RECOVERY_SNAPSHOT"
    if snapshot.exists():
        raise T04Error("POST_RECOVERY_SNAPSHOT_PREEXISTS")
    ...
    body = {
        ...
        "acceptance_command": (
            "TZ=America/Los_Angeles npm run test:dashboard-date && "
            "TZ=UTC npm run test:dashboard-date && npm run typecheck && npm run build"
        ),
...
def main() -> int:
    ...
    try:
        ...
        if args.phase == "preflight":
            return preflight()
        return execute()
    except (OSError, ValueError, json.JSONDecodeError, subprocess.TimeoutExpired, T04Error) as exc:
        ...
        if args.phase == "execute":
            try:
                ...
                pass
        print(canonical({"status": "EV1_T04_BLOCKED", "phase": args.phase, "reason": reason}).decode())
        return 1

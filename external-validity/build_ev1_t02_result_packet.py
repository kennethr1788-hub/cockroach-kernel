#!/usr/bin/env python3
"""Build the sanitized EV1-T02 operator-observation evidence audit packet."""
...
TYPECHECK_LOG = CONTROL / "t02-successor-typecheck.log"
BUILD_LOG = CONTROL / "t02-successor-build.log"
STORAGE_LOG = CONTROL / "t02-successor-storage-contract.log"
OBSERVATION = ROOT / "EXTERNAL_VALIDITY_EV1_T02_OPERATOR_OBSERVATION_R1.md"
MECHANICAL = ROOT / "EXTERNAL_VALIDITY_EV1_T02_MECHANICAL_RESULT_R1.md"
BODY = CONTROL / "EV1_T02_RESULT_AUDIT_BODY_R1.md"
PACKET = CONTROL / "EV1_T02_RESULT_AUDIT_PACKET_R1.md"
...
def main() -> int:
    required = (WORK, RESULT, TYPECHECK_LOG, BUILD_LOG, STORAGE_LOG, OBSERVATION, MECHANICAL)
    ...
    if work.get("state_mix", {}).get("committed") != ["scripts/run-storage-contract.mjs"]:
        raise RuntimeError("PRE_LOSS_COMMITTED_SET_DRIFT")

    parts = [
        "# EV1-T02 Operator-Observation Evidence Audit Body R1\n",
        ...
        "1. Does the evidence support that all three declared T02 files were restored byte-exact "
        "into a fresh no-Git-history successor and that typecheck, build, and the storage contract "
        "passed, making productive continuation mechanically demonstrated?\n"
        "2. Does the pre-loss Git evidence support that task commit "
        "`769321ec9828948afdacc7856321495c0ffd40a6` included only the committed runner among the "
        ...
        f"- Storage-contract log SHA-256: `{sha256(STORAGE_LOG.read_bytes())}`\n",
        ...
        fenced("Raw successor storage-contract output", "text", STORAGE_LOG.read_text()),
    ]
    body_raw = ("\n".join(parts).rstrip() + "\n").encode("utf-8")

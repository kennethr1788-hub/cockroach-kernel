#!/usr/bin/env python3
"""Tear down only the bounded EV1-T02 temporary successor after final audit."""
...
CAMPAIGN = ROOT / ".ev1-runtime" / "EV1-T02"
CONTROL = CAMPAIGN / "control"
TARGET = Path("/private/tmp/ck-ev1-t02-r1")
SNAPSHOT = CONTROL / "POST_RECOVERY_SNAPSHOT"
TASK_RECEIPT = CONTROL / "TASK_EXECUTION_RECEIPT.json"
AUDIT_RAW = CONTROL / "EV1_T02_RESULT_AUDIT_GLM_RAW_R1.txt"
TEARDOWN_RECEIPT = CONTROL / "TEARDOWN_RECEIPT.json"
EXPECTED_TASK_FILE_SHA256 = "40a5b3c7342060b739ae2a60f3108dd6cbe65422f7bf5d72a7eb785c8db9204d"
EXPECTED_TASK_RECEIPT_SHA256 = "fe18b5b10cb4d2174ff8fefc3b9dfd606bc59470a356aca93fad880cd4f846cf"
EXPECTED_AUDIT_RAW_SHA256 = "14cf78552e916136d9204b5ac05b847e3186fe824bd7b7175771b24f347c3481"
...
def main() -> int:
    ...
    if sorted(path.name for path in TARGET.iterdir()) != ["empty-home", "recovery"]:
    ...
    processes_before = related_processes()
    if processes_before:
        raise RuntimeError("TEARDOWN_RELATED_PROCESS_PRESENT")
    open_before = open_files()
    if open_before:
    ...
    processes_after = related_processes()
    if processes_after:
        raise RuntimeError("POST_TEARDOWN_RELATED_PROCESS_PRESENT")

    body = {
        "version": "ev1-t02-teardown-receipt-v1",
        "status": "EV1_T02_TEMPORARY_SUCCESSOR_TEARDOWN_GREEN",
        "task_id": "EV1-T02",
        "exact_teardown_root": "/private/tmp/ck-ev1-t02-r1",

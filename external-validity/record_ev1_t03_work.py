#!/usr/bin/env python3
"""Verify and freeze EV1-T03 task work before capture."""
...
EXPECTED_PREPARATION_FILE_SHA256 = "9454933b309121e7a8e4557dca0f7e444a90870fbc386f8bf58c426b5c46904a"
EXPECTED_PREPARATION_RECEIPT_SHA256 = "595b420b43988da1179e386d23dd34dbffd5aa97233ba1f1edc1ba266ec5dedc"
EXPECTED_TASK_COMMIT = "b18edb6f9b2b1c126e38c6fe218a167fe7ac7ca4"
DECLARED = (
    "package.json",
...
def preparation() -> dict[str, Any]:
    ...
    if value.get("status") != "EV1_T03_READY_FOR_AUTONOMOUS_TASK_WORK":
...
def changed_paths(prepared: dict[str, Any]) -> dict[str, list[str]]:
    ...
    expected = {
        ...
        "status": [" M package.json", "?? scripts/recipe-invariant-cases.cjs"],
    }
    if result != expected:
...
def main() -> int:
    if WORK_RECEIPT.exists():
        raise WorkError("EV1_T03_WORK_ALREADY_RECORDED")
    ...
    repeats = [
        offline(
            ["/usr/local/bin/npm", "run", "test:recipe-invariants"],
            f"work-recipe-invariants-repeat-{index}",
        )
    ...
    temporary_residue = sorted(
        path.name for path in (CONTROL / "tmp").glob("brew-ledger-recipe-invariants-*")
    )
    if temporary_residue:
        raise WorkError("TASK_TEST_TEMP_RESIDUE")

    body = {
        "version": "ev1-t03-work-receipt-v1",
        ...
        "acceptance": {"typecheck": typecheck, "build": build, "recipe_invariants": test},
        "determinism": {
            "executions": 5,
...
if __name__ == "__main__":
    try:
        ...
        print(canonical({"status": "EV1_T03_WORK_BLOCKED", "reason": str(exc)}).decode())
        raise SystemExit(1)

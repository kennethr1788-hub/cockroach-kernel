#!/usr/bin/env python3
"""Run and validate the two frozen EV1-T05 pre-execution judge lanes."""
...
GLM_RAW = CONTROL / "EV1_T05_EXECUTION_PREFLIGHT_GLM_RAW_R3.txt"
AGY_RAW = CONTROL / "EV1_T05_EXECUTION_PREFLIGHT_AGY_RAW_R1.txt"
...
def validate_glm(raw: bytes) -> None:
    ...
    required_patterns = (
        ...
        rf"review[- ]content(?:-sha256| hash):\s*{REVIEW_CONTENT_SHA256}",
    ...
    if not re.search(r"Concrete[- ]Blockers:\s*(?:None|\[\])", text, re.IGNORECASE):
        raise JudgeError("GLM_BLOCKERS_NOT_EMPTY")
    if not re.search(r"Evidence[- ]Gaps:\s*(?:None|\[\])", text, re.IGNORECASE):

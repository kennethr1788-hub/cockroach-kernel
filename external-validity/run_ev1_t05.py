#!/usr/bin/env python3
"""Capture and execute the frozen EV1-T05 recovery campaign.
...
PREFLIGHT_CAMPAIGN = Path("/private/tmp/ck-ev1-t05-preflight-r1")
EXECUTION_CAMPAIGN = Path("/private/tmp/ck-ev1-t05-r1")
...
BASELINE_COMMIT = "898f50d624c62ea4d54900c680d7ff362cc80e84"
...
AUTHORIZATION_FILE_SHA256 = "a6093f2c6a7afda7487b1627b7ebcd16593998df1dc88f2ac8870d1eeae94204"
PREPARATION_FILE_SHA256 = "43836cc3bb9a6f8cfdd730c1c199cbe165a79b8d4afd73c2ebf842eac82c701f"
PREPARATION_INTERNAL_SHA256 = "5236b6f4e65195579f54976e1cf76ac4e61be332650f654aa69000d1e49f893a"
WORK_RECEIPT_FILE_SHA256 = "417323949ead439e9dd42aa2343fe607edd3a3f6cb25475f8382245b8994cf98"
WORK_RECEIPT_INTERNAL_SHA256 = "bb312c593c1a1b41ca6b2c4dbce6bdc36b459ae787328d94a6a6451e8d999928"
TASK_COMMIT = "63f151a50d6e4b28cc2091f22c045d785c0261c1"
CAPTURE_DECLARATION = (
    "I, Kenneth, explicitly declare the exact current EV1-T05 state—committed "
    "lib/signalSchema.ts and scripts/run-signal-schema.mjs at task commit "
    "63f151a50d6e4b28cc2091f22c045d785c0261c1, modified lib/signals.ts and "
    "package.json, and untracked scripts/signal-schema-cases.cjs—permitted for "
    "capture, guarded disposable-workspace deletion, and fresh-process recovery "
    "under the frozen EV1 protocol."
)
CAPTURE_DECLARATION_UTC = "2026-07-30T18:53:35Z"
DECLARED = (
    "lib/signalSchema.ts",
    "lib/signals.ts",
    "package.json",
...
EXPECTED_LOGS = {
    "work-signal-schema.log": "229c21d068f1496925f83d8331ccffca76c474f9f516169ff05a3fd808701ae4",
    "work-typecheck.log": "8c0af875a1ab948857b68d4f22b66e9bce86deedfdf47d7ba6ea1d528e01bbda",
    "work-build.log": "94e24c8e6c8a0f3e4994e2ae4c842d0605ad5e89feb9a832de07fa9132009226",
}
...
def load_t04() -> Any:
    if hashlib.sha256(T04_RUNNER.read_bytes()).hexdigest() != T04_RUNNER_SHA256:
        raise T05Error("T04_PRIMITIVE_SOURCE_DRIFT")
    spec = importlib.util.spec_from_file_location("ev1_t04_frozen", T04_RUNNER)
    if spec is None or spec.loader is None:
        raise T05Error("T04_PRIMITIVE_IMPORT_FAILED")
...
def verify_work_state() -> dict[str, Any]:
    ...
    if hashes != EXPECTED_HASHES or receipt.get("declared_file_hashes") != EXPECTED_HASHES:
        raise T05Error("TASK_FILE_HASH_DRIFT")
    for name, expected in EXPECTED_LOGS.items():
        if digest(CONTROL / name) != expected:
            raise T05Error(f"PRE_LOSS_LOG_DRIFT:{name}")
    authorization = ROOT / "EXTERNAL_VALIDITY_EV1_T05_CAPTURE_AUTHORIZATION_R1.md"
    if digest(authorization) != AUTHORIZATION_FILE_SHA256:
        raise T05Error("CAPTURE_AUTHORIZATION_DRIFT")
...
def make_request(r3: Any, file_hashes: dict[str, str]) -> dict[str, Any]:
    ...
    manifest = {
        "version": p7.VERSION,
        "manifest_id": "manifest-ev1-t05-r1",
        "task_id": TASK_ID,
        "files": [
            {
                "path": path,
    ...
    labels = (
        ...
        "COMMITTED_STRICT_SCHEMA_PRESENT",
        ...
        "ACTUAL_DATASET_AND_OFFLINE_ACCEPTANCE_GREEN",
    )
    events = [
    ...
    trajectory = {
        "version": p7.VERSION,
        "receipt_id": "trajectory-ev1-t05-r1",
        "task_id": TASK_ID,
    ...
    candidate = {
        "version": p7.VERSION,
        "candidate_id": "candidate-ev1-t05-r1",
        "task_id": TASK_ID,
        ...
        "executable_test": {
            "test_id": "test-ev1-t05-signal-schema-r1",
            "path": "scripts/signal-schema-cases.cjs",
            "feature_hash": file_hashes["scripts/signal-schema-cases.cjs"],
            "passed": True,
        },
    }
    decision = p7.select_candidate([candidate], context)
    if decision.get("decision") != "PROMOTE":
        raise T05Error("PREPARED_CANDIDATE_NOT_ADMITTED")
    warrant = p7.make_warrant("warrant-ev1-t05-r1", TASK_ID, candidate["candidate_id"], decision)
    loss = {
        "version": p7.VERSION,
        "receipt_id": "loss-ev1-t05-r1",
        "task_id": TASK_ID,
        "manifest_hash": p7.sha256_hex(manifest),
        "lost_paths": paths,
...
def verify_product(r3: Any) -> tuple[Path, Path, Path]:
    if digest(r3.PROFILE) != EXPECTED_PROFILE_SHA256:
        raise T05Error("SEATBELT_PROFILE_HASH_MISMATCH")
    try:
        return T04.verify_product(r3)
    except RuntimeError as exc:
        raise T05Error(f"PRODUCT_RUNTIME_INVALID:{exc}") from exc
...
def dependency_shape(root: Path) -> dict[str, Any]:
    try:
        ...
        raise T05Error(f"DEPENDENCY_SHAPE_INVALID:{exc}") from exc
...
def copy_representations_into(destination: Path) -> None:
    candidate = REPRESENTATIONS / "candidate-ev1-t05-r1"
...
def capture() -> int:
    if CAPTURE_RECEIPT.exists() or TASK_RECEIPT.exists() or FAILURE_RECEIPT.exists():
        raise T05Error("T05_CAPTURE_ALREADY_STARTED")
    if any(
        path.exists()
        for path in (
            RECOVERY,
        ...
        raise T05Error("PREEXISTING_CAMPAIGN_ROOT")
    ...
    atomic_bytes(outside, b"EV1-T05 outside kill canary\n")
    body = {
        "version": "ev1-t05-capture-receipt-v1",
        "status": "EV1_T05_CAPTURE_GREEN_EXECUTION_NOT_STARTED",
        "task_id": TASK_ID,
        ...
        "kill_target": ".ev1-runtime/EV1-T05/workspace",
...
def run_acceptance(workspace: Path, prefix: str, tmpdir: Path) -> dict[str, Any]:
    ...
    commands = (
        (f"{prefix}-signal-schema", ["/usr/local/bin/npm", "run", "test:signal-schema"], 300),
...
def preflight() -> int:
    ...
    r3.make_fixture(product_canary, "ev1-t05-predelete")
    ...
    if baseline_files != 18 or (dependency_workspace / ".git").exists():
        raise T05Error("DEPENDENCY_CANARY_BASELINE_INVALID")
    ...
    acceptance = run_acceptance(dependency_workspace, "t05-dependency-preflight", dependency_tmp)
    hashes = {relative: digest(safe_file(dependency_workspace, relative)) for relative in DECLARED}
    if hashes != EXPECTED_HASHES:
        raise T05Error("DEPENDENCY_CANARY_WORK_UNITS_MISMATCH")

    shutil.rmtree(PREFLIGHT_CAMPAIGN)
    if PREFLIGHT_CAMPAIGN.exists():
        raise T05Error("PREFLIGHT_TEARDOWN_FAILED")
    body = {
        "version": "ev1-t05-execution-preflight-receipt-v1",
        ...
        "execution_root": "/private/tmp/ck-ev1-t05-r1",
...
def guarded_destroy() -> dict[str, Any]:
    ...
    if ORIGINAL != expected or ORIGINAL.is_symlink() or not ORIGINAL.is_dir():
        raise T05Error("KILL_TARGET_MISMATCH")
...
def preserve_failure(reason: str) -> None:
    ...
    if not FAILURE_RECEIPT.exists():
        atomic_record(
            FAILURE_RECEIPT,
            {
                "version": "ev1-t05-failure-receipt-v1",
                "status": "EV1_T05_BLOCKED",
                "reason": reason,
...
def execute() -> int:
    if TASK_RECEIPT.exists() or FAILURE_RECEIPT.exists():
        raise T05Error("T05_EXECUTION_ALREADY_TERMINAL")
    captured = load_receipt(CAPTURE_RECEIPT)
    preflighted = load_receipt(PREFLIGHT_RECEIPT)
    if captured.get("status") != "EV1_T05_CAPTURE_GREEN_EXECUTION_NOT_STARTED":
        raise T05Error("CAPTURE_STATUS_INVALID")
    if preflighted.get("status") != "EV1_T05_EXECUTION_PREFLIGHT_GREEN_DELETION_NOT_STARTED":
        raise T05Error("EXECUTION_PREFLIGHT_STATUS_INVALID")
    if preflighted.get("capture_file_sha256") != digest(CAPTURE_RECEIPT):
        raise T05Error("PREFLIGHT_CAPTURE_DRIFT")
    if preflighted.get("runner_sha256") != digest(Path(__file__).resolve()):
        raise T05Error("RUNNER_DRIFT")
    if verify_work_state() != captured.get("declared_state"):
        raise T05Error("PRE_EXECUTION_STATE_DRIFT")
    if digest(REQUEST) != captured.get("request_sha256"):
        raise T05Error("REQUEST_DRIFT")
    if tree_hashes(REPRESENTATIONS) != captured.get("representation_hashes"):
        raise T05Error("REPRESENTATION_DRIFT")
    if tree_hashes(BASELINE_SNAPSHOT) != captured.get("baseline_snapshot_hashes"):
        raise T05Error("BASELINE_SNAPSHOT_DRIFT")
    if EXECUTION_CAMPAIGN.exists() or DEPENDENCY_RUNTIME.exists():
        raise T05Error("EXECUTION_ROOT_PREEXISTS")

    r3 = load_r3()
    toolchain, venv, entrypoint = verify_product(r3)
    if digest(entrypoint) != captured.get("product_entrypoint_sha256"):
        raise T05Error("PRODUCT_ENTRYPOINT_DRIFT")
    ...
    if baseline_files != 18 or (EXECUTION_SUCCESSOR / ".git").exists():
        raise T05Error("SUCCESSOR_BASELINE_INVALID")
    ...
    if successor_dependency != dependency:
        raise T05Error("SUCCESSOR_DEPENDENCY_DRIFT")
    ...
    recovery_record = record_command("t05-recovery", recovered)
    if recovered.returncode != 0:
        raise T05Error(f"PRODUCT_RECOVERY_FAILED:{recovered.returncode}")
    summary = json.loads(recovered.stdout)
    if summary.get("verdict") != "PROMOTE" or summary.get("fresh_context_continued") is not True:
        raise T05Error("PRODUCT_RECOVERY_NOT_PROMOTED")
    if tree_hashes(EXECUTION_REPRESENTATIONS) != captured["representation_hashes"]:
        raise T05Error("EXECUTION_REPRESENTATION_MUTATED")
    if tree_hashes(REPRESENTATIONS) != captured["representation_hashes"]:
        raise T05Error("AUTHORITATIVE_REPRESENTATION_MUTATED")
    if tree_hashes(BASELINE_SNAPSHOT) != captured["baseline_snapshot_hashes"]:
        raise T05Error("AUTHORITATIVE_BASELINE_MUTATED")

    acceptance = run_acceptance(EXECUTION_SUCCESSOR, "t05-successor", EXECUTION_TEMP)
    ...
    if restored != EXPECTED_HASHES:
        raise T05Error("SUCCESSOR_WORK_UNITS_MISMATCH")
    process_scan = run(["ps", "-axo", "pid=,command="], cwd=ROOT, timeout=30)
    if process_scan.returncode != 0:
        raise T05Error("PROCESS_RESIDUE_SCAN_FAILED")
    ...
    for raw_line in process_scan.stdout.splitlines():
        ...
        if len(fields) == 2 and fields[0].isdigit() and int(fields[0]) != os.getpid():
            if any(marker in fields[1] for marker in markers):
                residue.append(line)
    if residue:
        raise T05Error("TASK_PROCESS_RESIDUE")

    snapshot = CONTROL / "POST_RECOVERY_SNAPSHOT"
    if snapshot.exists():
        raise T05Error("POST_RECOVERY_SNAPSHOT_PREEXISTS")
    ...
    body = {
        ...
        "acceptance_command": "npm run test:signal-schema && npm run typecheck && npm run build",
        "acceptance": acceptance,
...
def main() -> int:
    ...
    try:
        ...
        if arguments.phase == "preflight":
            return preflight()
        return execute()
    except (OSError, ValueError, json.JSONDecodeError, subprocess.TimeoutExpired, T05Error) as exc:
        ...
        if arguments.phase == "execute":
            try:
                ...
                pass
        print(canonical({"status": "EV1_T05_BLOCKED", "phase": arguments.phase, "reason": reason}).decode())
        return 1

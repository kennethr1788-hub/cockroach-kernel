#!/usr/bin/env python3
"""Prepare the source-bound disposable EV1-T04 workspace."""
...
DEPENDENCY_SOURCE = ROOT / ".ev1-runtime" / "EV1-T03" / "dependency-runtime" / "node_modules"
...
T03_TEARDOWN = ROOT / ".ev1-runtime" / "EV1-T03" / "control" / "TEARDOWN_RECEIPT.json"
T03_TEARDOWN_FILE_SHA256 = "49fdf0f8dde61e8f403d2c2ef6243d9581c443ef78c64da9d16ee886ac889984"
...
def initialize_baseline() -> str:
    commands = (
        ...
        ["add", "-A"],
        ["commit", "-m", "Bind EV1-T04 source baseline"],
    )
...
def clone_dependencies(t02: Any) -> dict[str, Any]:
    ...
    return {
        "source_relative": ".ev1-runtime/EV1-T03/dependency-runtime/node_modules",
...
def main() -> int:
    if CAMPAIGN.exists() or CAMPAIGN.is_symlink():
        raise PreparationError("EV1_T04_CAMPAIGN_ALREADY_EXISTS")
    if digest(T03_TEARDOWN) != T03_TEARDOWN_FILE_SHA256:
        raise PreparationError("T03_TEARDOWN_DRIFT")
    ...
    pacific_env = dict(env, TZ="America/Los_Angeles")
    utc_env = dict(env, TZ="UTC")
    missing_pacific = offline(
        ["/usr/local/bin/npm", "run", "test:dashboard-date"],
        pacific_env,
        "baseline-dashboard-date-pacific",
    )
    missing_utc = offline(
        ["/usr/local/bin/npm", "run", "test:dashboard-date"],
        utc_env,
        "baseline-dashboard-date-utc",
    )
    if (
        typecheck["exit"] != 0
        or build["exit"] != 0
        or missing_pacific["exit"] == 0
        or missing_utc["exit"] == 0
    ):
    ...
    body = {
        "version": "ev1-t04-preparation-receipt-v1",
        "status": "EV1_T04_READY_FOR_AUTONOMOUS_TASK_WORK",
        "task_id": TASK_ID,
        ...
        "workspace_relative": ".ev1-runtime/EV1-T04/workspace",
        "objective": (
            "Extract the dashboard local-calendar-day selection into a pure helper and add deterministic "
            "timezone cases proving a late-Pacific brew remains on the correct local day."
        ),
        "acceptance_commands": [
            "TZ=America/Los_Angeles npm run test:dashboard-date",
            "TZ=UTC npm run test:dashboard-date",
            "npm run typecheck",
            "npm run build",
        ],
        ...
        "baseline": {
            "typecheck": typecheck,
            "build": build,
            "dashboard_date_pacific_absent": missing_pacific,
            "dashboard_date_utc_absent": missing_utc,
        },

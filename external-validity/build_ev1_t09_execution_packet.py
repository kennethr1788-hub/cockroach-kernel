#!/usr/bin/env python3
"""Build the byte-frozen EV1-T09 guarded-execution judge packet."""
...
RUNNER = ROOT / "external-validity" / "run_ev1_t09.py"
...
LOGS = (
    CONTROL / "t09-product-preflight.log",
    CONTROL / "t09-dependency-preflight-prettier.log",
    CONTROL / "t09-dependency-preflight-policy.log",
)
BODY = CONTROL / "EV1_T09_EXECUTION_PREFLIGHT_BODY_R1.md"
...
def main() -> int:
    required = (RUNNER, AUTHORIZATION, AMENDMENT, CAPTURE, PREFLIGHT, *LOGS)
    ...
    if capture.get("independent_human_edit") is not False:
    ...
    capture_projection = {
        "version": "ev1-t09-sanitized-capture-projection-v1",
    ...
    body_parts = [
        "# EV1-T09 Guarded Execution Review Body R1\n",
        ...
        "of only the disposable EV1-T09 workspace, create an empty-history successor from the "
        "separately attributed ordinary-Git baseline, recover exactly three hash-bound task files "
        "through the unchanged product candidate, run the exact pinned formatter and release-policy "
        "validator offline, preserve failures, and stop for operator observation. The policy sentence "
        "is model-assisted and must never be credited as an independently human-authored edit.\n",
        "## Blocking criteria\n\n"
        ...
        "misreported as recovered task work; model-assisted work misreported as independent-human "
        "evidence; missing failure preservation; acceptance mismatch; undeclared second execution; or "
        ...
        "- Edit classification: `MODEL_ASSISTED`\n"
        "- Independent human edit: `FALSE`\n"
        "- Independent-human-edit claim: `PERMANENTLY_EXCLUDED_FOR_EV1_T09`\n"
        ...
        fenced("Model-assisted classification amendment", "markdown", AMENDMENT.read_text()),
        fenced(

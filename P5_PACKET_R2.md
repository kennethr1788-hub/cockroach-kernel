...
- Judge verdicts are external receipts and are intentionally absent from this frozen evidence packet.
...
RECUSAL_CHECK:
```
- Each judge must evaluate its assigned domain independently from this packet. A judge must not block merely because its peer verdict is pending or external; the orchestrator combines same-hash verdicts after both runs.
